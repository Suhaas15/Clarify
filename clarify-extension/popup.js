...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const fromCtxEl = /** @type {HTMLInputElement|null} */ (document.getElementById('fromContext'))
    || /** @type {HTMLInputElement|null} */ (document.getElementById('onlyContext'));
  ...
  if (!fromCtxEl) console.warn('[Clarify] Checkbox not found: #fromContext or #onlyContext');
  if (!sendBtn) console.error('[Clarify] Missing #sendBtn');
  ...
  sendBtn.addEventListener('click', () => {
    (async function handleSendClick() {
      const body = {
        ...
        fromContext: !!fromCtxEl?.checked,
        max: Math.max(64, Math.min(4096, Number(maxTokEl?.value || DEFAULT_MAX_TOKENS))),
      };
